...
public class SearchView implements Serializable {
	...
	public void doSearch(String method) {
		...
		switch (selectedFilterType) {
			case ticket:
			case wiki:
				builder.addStrongRestriction(selectedFilterType.name(), TracStuffField.CONTEXT);
		}
		
		if (!selectedFilterPeriod.equals(FilterPeriodSelection.all_entries)) {
			builder.enableRecentFilter(selectedFilterPeriod.days);
		}
		
		switch (searchMethod) {
			case precise:
				builder.addWeakRestriction(searchInfo.getSearchText(), TracStuffField.CONTENT);
				break;
	
			case advanced:
				builder.addLuceneRestriction(searchInfo.getSearchText());
				break;
		...
		results.addAll(getSortedResults(manager.doSearch(query)));

		long elapsedTime = System.currentTimeMillis() - initSearch;
		
		searchInfo.setInterpretedQuery(query.toString());
		searchInfo.setElapsedTime(decFormat.format((double)elapsedTime / 1000D));
		
		usageStatistics.logSearch(userid, query.toString(), elapsedTime);
